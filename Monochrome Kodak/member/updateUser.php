...
<body>
  ...
  <?php
		...
		$sql ="SELECT * FROM `member` WHERE `email`='".$_SESSION['member']."'";	
  ...
  ?>
    ...
    <div class="container bg-light" style="justify-items: center; margin:50px; padding:20px; border-radius: 20px;">
      <h2 class="text-warning" style="margin-bottom: 20px">| Update Member</h2>
      <form action="">
        
        <div class="mb-3" style="width:30%">
          <label for="email" class="form-label text-warning fw-bold">E-mail:</label>
          <input type="email" class="form-control" name="email" id="email" required="required" value="<?php echo $row['email'];?>">
        </div>
        ...
        <div class="mb-3" style="width:30%">
            ...
            <input type="password" class="form-control" id="password" name="password" required="required" value="<?php echo $row['pass'];?>">
        </div>

        <div class="mb-3" style="width:30%">
            <label for="phone" class="form-label text-warning fw-bold">Mobile Number:</label>
            <input type="number" class="form-control" id="phone" name="phone" required="required"  min="10" value="<?php echo $row['phone'];?>">
        </div>
  
        <div class="mb-3" style="width:30%">
              <label for="Address" class="form-label text-warning fw-bold">Address:</label>
              <input type="text" class="form-control" id="add" name="add" required="required" value="<?php echo $row['address'];?>">
        </div>

        <div class="mb-3" style="width:30%">
            <label for="city" class="form-label text-warning fw-bold">City:</label>
            <input type="text" class="form-control" id="city" name="city" required="required" pattern="[a-zA-Z\s]+" value="<?php echo $row['city'];?>">
        </div>

        <div class="mb-3" style="width:30%">
          <label for="Zip" class="form-label text-warning fw-bold">Zip:</label>
          <input type="number" class="form-control" id="zip" name="zip" required="required" value="<?php echo $row['zip'];?>">
      </div>

        <button type="submit" class="btn btn-warning text-light fw-bold">Update</button>
        ...
        $sql= "UPDATE `member` SET `name` = '$name', `email` = '$email', `phone` = '$phone', `pass` = '$pass', `address` = '$add', `city` = '$city', `zip` = '$zip' WHERE `member`.`email` = ".$_GET['email'].";";
